...
function renderDatabase() {
  ...
  $('.db-indicator').style.background = status.online ? 'var(--lime)' : 'var(--red)';
...
$('#key-form').addEventListener('submit', async (event) => {
  event.preventDefault();
  try {
    const license = await api('/api/keys', { method: 'POST', body: JSON.stringify({ productId: $('#key-product').value, days: Number($('#key-days').value), maxActivations: Number($('#key-activations').value) }) });
    state.keys.unshift(license);
    render();
    const box = $('#generated-key');
    box.querySelector('code').textContent = license.key;
...
$('#generated-key button').addEventListener('click', async () => {
  await navigator.clipboard.writeText($('#generated-key code').textContent);
  toast('Key copiada');
});
