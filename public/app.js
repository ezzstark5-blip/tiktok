...
function renderLicenseTable() {
  ...
  const rows = keys.map((key) => `<tr><td><code>${escapeHtml(key.key)}</code></td><td>${escapeHtml(key.productId)}</td><td><span class="status ${key.status}">${key.status === 'active' ? 'ATIVA' : 'BLOQUEADA'}</span></td><td>${key.activations.length}/${key.maxActivations}</td><td>${formatDate(key.expiresAt)}</td><td><button class="action-btn" data-key-id="${key.id}" data-status="${key.status === 'active' ? 'disabled' : 'active'}">${key.status === 'active' ? 'Bloquear' : 'Ativar'}</button></td></tr>`).join('');
...
$('#key-search').addEventListener('input', renderLicenseTable);
$('#key-status-filter').addEventListener('change', renderLicenseTable);
$('#user-search').addEventListener('input', renderDatabaseUsers);
$('#user-role-filter').addEventListener('change', renderDatabaseUsers);
$('#db-key-search').addEventListener('input', renderDatabaseKeys);
