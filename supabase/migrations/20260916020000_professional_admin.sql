...
begin
  loop
    ...
    set key_hash = encode(extensions.digest(convert_to(upper(trim(license_key)), 'UTF8'), 'sha256'), 'hex')
...
begin
  ...
  where key_hash = encode(extensions.digest(convert_to(upper(trim(p_key)), 'UTF8'), 'sha256'), 'hex') for update;
