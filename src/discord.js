...
const { createKey, listProducts } = require('./licenses');
  ...
  client.on('interactionCreate', async (interaction) => {
    ...
    try {
      if (interaction.commandName === 'produtos') {
        ...
        await interaction.editReply(products.map((p) => `**${p.name}** (\`${p.id}\`) — R$ ${p.price.toFixed(2)}`).join('\n'));
        return;
      }
      ...
      if (interaction.commandName === 'gerarkey') {
        ...
        const expirationTimestamp = Math.floor(new Date(license.expiresAt).getTime() / 1000);
      ...
      const usesComponentsV2 = interaction.commandName === 'gerarkey' || interaction.commandName === 'gerarusuario';
      try {
