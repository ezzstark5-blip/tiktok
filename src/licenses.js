...
async function createKey({ productId, days = 30, maxActivations = 1, createdBy = 'api' }) {
  ...
  const license = {
    ...
    createdAt: now.toISOString(), expiresAt: new Date(now.getTime() + parsedDays * 86400000).toISOString()
  };
  const { error } = await db.from('kf_license_keys').insert({
...
module.exports = { createKey, validateKey, redeemKey, listUserProducts, listProducts, setKeyStatus };
