...
// Dupa un esec de init, nu mai bombardam baza la fiecare request —
// asteptam INIT_RETRY_COOLDOWN_MS inainte sa reincercam. Intre timp
// request-urile pica rapid cu 503 retryable in loc sa faca pile-up
// de query-uri lente (care agraveaza statement timeout-ul).
const INIT_RETRY_COOLDOWN_MS = 15000;

function dbUnavailable() {
  const error = new Error('Supabase: baza de date indisponibila temporar, reincercati.');
  error.status = 503;
...
function fail(error) {
  ...
  if (/statement timeout|57014|canceling statement/i.test(text)) {
    const timeoutError = new Error('Supabase temporariamente sobrecarregado. A consulta foi cancelada e pode ser repetida em alguns segundos.');
  ...
  const genericError = new Error(`Supabase: ${error.message || 'falha na consulta'}`);
  genericError.status = 503;
  genericError.code = error.code || 'SUPABASE_ERROR';
...
async function ensureDatabase() {
  ...
  if (Date.now() - lastInitFailureAt < INIT_RETRY_COOLDOWN_MS) dbUnavailable();
...
async function getAllKeys({ limit = 100 } = {}) {
  await ensureDatabase();
  ...
  const { data: keys, error } = await db.from('kf_license_keys')
    .select('id,key_hint,license_key,product_id,status,max_activations,created_by,redeemed_by_user_id,notes,created_at,updated_at,expires_at')
    // Sem ORDER BY: esta rota e apenas de compatibilidade. Ordenar por
    // created_at antes do indice da migration 300 causa scan+sort e timeout.
    .limit(safeLimit);
  fail(error);
  if (!keys?.length) return [];
  // PostgREST serializa .in() na URL. Com centenas de UUIDs uma unica query
  // ultrapassa o limite do proxy e retorna HTTP 400. Blocos de 50 mantem a
  // URL curta e cada consulta usa idx_kf_activations_license.
  const activations = [];
  const ids = keys.map((item) => item.id);
  for (let index = 0; index < ids.length; index += 50) {
    const batchIds = ids.slice(index, index + 50);
    let result = await db.from('kf_activations')
      .select('license_id,hwid_hash,activation_ip,activated_at,last_seen_at')
      .in('license_id', batchIds);
    // Compatibilidade durante o deploy da migration que adiciona activation_ip:
    // o cache do PostgREST pode levar alguns segundos para reconhecer a coluna.
    if (result.error && /activation_ip|column|schema cache/i.test(`${result.error.message || ''} ${result.error.details || ''}`)) {
      result = await db.from('kf_activations')
        .select('license_id,hwid_hash,activated_at,last_seen_at').in('license_id', batchIds);
    }
    fail(result.error);
    activations.push(...(result.data || []));
  }
...
async function getAdminOverview() {
  ...
  const db = getClient();
  const [usersResult, keysResult, statusResult, auditResult] = await Promise.allSettled([
    db.from('kf_users').select('id,username,role,created_at').order('created_at').limit(500),
    listKeysPage({ page: 1, pageSize: 100, sort: 'id' }),
    getDatabaseStatus(), getAuditLogs({ limit: 30 })
  ]);
  const usersQuery = usersResult.status === 'fulfilled' ? usersResult.value : { data: [], error: usersResult.reason };
  if (usersQuery.error) console.error('[Supabase] Overview usuarios:', usersQuery.error.message || usersQuery.error);
  const keysPage = keysResult.status === 'fulfilled' ? keysResult.value : { items: [], total: 0, totalPages: 1 };
  const status = statusResult.status === 'fulfilled' ? statusResult.value : {
    online: false, latencyMs: 0, engine: 'PostgreSQL (Supabase REST)', database: 'postgres',
    host: new URL(process.env.SUPABASE_URL).hostname, ssl: true, tables: []
  };
  const auditLogs = auditResult.status === 'fulfilled' ? auditResult.value : [];
  return {
    status,
    users: (usersQuery.data || []).map((row) => ({ id: row.id, username: row.username, role: row.role, createdAt: iso(row.created_at) })),
...
async function listKeysPage({ page = 1, pageSize = 25, search = '', status = 'all', productId = 'all', sort = 'created_at', direction = 'desc' } = {}) {
  ...
  const allowedSort = new Set(['id', 'created_at', 'expires_at', 'status', 'product_id']);
  ...
  let query = getClient().from('kf_license_keys')
    .select('id,key_hint,license_key,product_id,status,max_activations,created_by,redeemed_by_user_id,notes,created_at,updated_at,expires_at', { count: 'estimated' });
