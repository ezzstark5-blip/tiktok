  ...
  try {
    ...
    try {
      ...
      if (migrationCountRows[0].count === 0) {
        const { rows: baselineRows } = await client.query(`select
          ...
          to_regclass('public.kf_audit_logs') is not null
            ...
            and exists (select 1 from information_schema.columns where table_schema='public' and table_name='kf_activations' and column_name='activation_ip') as v200,
      ...
      for (const migrationFile of migrationFiles) {
        ...
        // Re-emite SET + re-verifica lock-ul INAINTE de fiecare fisier:
        // pooler-ul Supavisor poate muta conexiunea pe alta sesiune
        // intre fisiere, iar SET-ul / lock-ul s-ar pierde tacit.
        await client.query("SET statement_timeout = '120s'").catch(() => {});
        const { rows } = await client.query(
          'SELECT pg_try_advisory_lock($1) AS locked', [ADVISORY_LOCK_ID]
        ).catch(() => ({ rows: [] }));
        if (!rows?.[0]?.locked) {
          console.log(`[Supabase] Lock detinut de alt proces — opresc migrarile la ${migrationFile} (reincerc la urmatorul boot).`);
          break;
        }
        haveLock = true;
