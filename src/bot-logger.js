...
function card(title, body, footer = 'KeyForge • Log automatico') {
...
async function upsert(kind, title, body) {
  ...
  return enqueue(channelId, async () => {
    ...
    const data = { components: [card(title, body, 'KeyForge • Painel atualizado automaticamente')], flags: MessageFlags.IsComponentsV2 };
    if (messageId) {
      try {
...
function logKeyBatch(action, licenses, actor = 'sistema') {
  ...
  const keyList = licenses.map((license, index) => `${index + 1}. \`${clean(license.key)}\``).join('\n');
