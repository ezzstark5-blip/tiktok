...
app.get('/api/health', async (_request, response) => {
  // Health-check-ul Render trebuie sa raspunda rapid cu 200 chiar daca
  // baza e inca in init sau picata — altfel platforma omoara/restart-eaza
  // serviciul in bucla ("No open ports detected" / unhealthy).
  // Timeout propriu de 4s: nu asteptam migrari lungi sau DB lent.
  let database;
  try {
    ...
    database = { online: false, latencyMs: 0, error: error.message };
  }
  response.json({
    ok: true,
...
app.get('/api/keys', requireAuth, requireAdmin, async (request, response) => {
  // Cap para nao estourar o statement_timeout: ?limit= (default 250, max 1000).
  // Para paginacao completa use GET /api/admin/keys?page=&pageSize=.
  const limit = Math.min(1000, Math.max(1, Number(request.query.limit) || 250));
...
app.use((error, _request, response, _next) => {
  const signature = `${error.code || 'ERROR'}:${error.message || 'Erro interno'}`;
  const lastLoggedAt = recentErrorLogs.get(signature) || 0;
  if (Date.now() - lastLoggedAt > 60000) {
    console.error(error);
    recentErrorLogs.set(signature, Date.now());
    if (recentErrorLogs.size > 100) recentErrorLogs.delete(recentErrorLogs.keys().next().value);
  }
...
async function main() {
  // IMPORTANT (Render "No open ports detected"): portul trebuie legat
  // INAINTE de orice asteptare pe baza de date. Daca ensureDatabase()
  // atarna (Supabase lent / statement timeout), procesul tot raspunde
  // pe PORT si health-check-ul trece; init-ul DB se face in fundal.
  ...
  // Init DB in fundal cu retry — nu blocheaza niciodata portul.
  // Daca pica, endpoint-urile intorc 503 retryable (vezi error handler).
  (async () => {
    for (let attempt = 1; ; attempt += 1) {
      try {
        ...
        console.error(`[Supabase] Init esuat (incercarea ${attempt}):`, error.message);
    ...
    try {
      ...
      console.error('[Discord] Bot indisponibil:', error.message);
    }
