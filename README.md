...
4. Abra `http://localhost:3000`. Nao existe senha padrao. Crie o primeiro acesso administrativo com `/gerarusuario` no Discord ou diretamente no Supabase.

Na inicializacao, as migrations sao aplicadas em ordem e os produtos padrao sao criados. Senhas usam `scrypt`, tokens de sessao e keys usam apenas hashes. A key completa e devolvida uma unica vez ao criador; o banco e as consultas administrativas exibem somente um identificador mascarado.
...
O painel oferece geracao assincrona de ate 5.000 keys com progresso, console visual, graficos reais, tema claro/escuro, pesquisa, filtros, selecao e acoes em massa, CSV/JSON, edicao, renovacao, historico e confirmacao dupla para exclusoes. A categoria `Database` mostra conexao, latencia, tabelas, usuarios, keys e auditoria.

Papeis: `viewer` consulta; `support` consulta e bloqueia; `operator` tambem cria; `admin` gerencia keys e usuarios; `superadmin` e reservado para configuracoes criticas. Toda autorizacao e validada novamente no backend.
...
| `POST` | `/api/keys/batch/jobs` | Operador+ | Iniciar geracao assincrona com progresso |
| `GET` | `/api/keys/batch/jobs/:id` | Operador+ | Consultar progresso e resultado temporario |
| `PATCH` | `/api/keys/:id/status` | Bearer | Ativar/bloquear key |
| `GET` | `/api/admin/keys` | Visualizador+ | Pesquisa e paginacao no servidor |
| `PATCH` | `/api/admin/keys/:id` | Admin | Editar validade, limite e observacoes |
| `GET` | `/api/admin/keys/:id/history` | Visualizador+ | Historico da key |
| `DELETE` | `/api/admin/keys/:id` | Admin | Excluir com confirmacao dupla |
| `POST` | `/api/admin/keys/bulk-action` | Suporte+ | Bloquear, reativar, revogar ou excluir selecao |
| `POST` | `/api/admin/keys/bulk-delete` | Admin | Excluir por quantidade e filtros |
| `GET` | `/api/admin/audit` | Visualizador+ | Consultar logs de auditoria |
...
| `PATCH` | `/api/admin/users/:id/role` | Admin | Alterar permissao e revogar sessoes |
| `PATCH` | `/api/admin/users/:id/password` | Admin | Redefinir senha e revogar sessoes |
| `POST` | `/api/admin/users/:id/revoke-sessions` | Admin | Encerrar sessoes do usuario |
| `DELETE` | `/api/admin/users/:id` | Admin | Excluir usuario protegido |

## Antes de publicar na internet

- Use HTTPS, mantenha `DEBUG_MODE=false` e rotacione os segredos periodicamente.
- O login bloqueia temporariamente depois de cinco falhas por usuario/IP.
- Em varios processos, mova o estado temporario dos jobs para Redis para compartilhar progresso entre instancias.
